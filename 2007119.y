...
%token<fv> NFLOAT
%token<iv> NINT
%token SINGLE_COMMENT MULTI_COMMENT
%type <iv> expre 
%type <fv> floatv
%nonassoc IF
%nonassoc ELSE
...
/*simple gram rules*/
%%
program: MAIN_FUNC LP RP LB firststatement RB    { printf("\n At last finished.... \n"); }
;
...
floatv : NFLOAT
         {
             store[indexx].tfloat=$1;
             $$=store[indexx].tfloat;
             indexx++;
         }
       | VARIABLE floattracker
         {
             int j=0,f=0;
             for(j=0;j<indexx;j++)
             {
